...
import { BaseModel, Filterable, FilterableType } from './types';
...
export const TableVerticalConfigs = <T extends BaseModel>({ callback, filters }: Props<T>) => {
    ...
    useEffect(() => {
        let queryString = queryStrings.get('size');
        if (queryString) {
    ...
    const showFiltersOnClickHandler = () => {
        MySwal.fire({
            ...
            width: 900,
            html: <ShowVerticalFilters callback={applyFilter} fields={filters! as { key: string, format: string, data?: { id: number, name: string }[], operation?: Operation, showOperations?: boolean }[]} />
        })
    }
    ...
    return (
        <>
            ...
            {queryStringsExist &&
                <button className="btn btn-sm btn-outline-danger rounded-circle m-1 float-right" onClick={onResetClickHandler}>
                    <RiFilterOffFill />
                </button>
            }
        </>
...
interface miniProps {
    ...
    fields: { key: string, format: string, data?: { id: number, name: string }[], operation?: Operation, showOperations?: boolean }[];
}
...
export const ShowVerticalFilters: React.FC<miniProps> = ({ callback, fields }) => {
    ...
    const onSubmit = (data: any, e: any) => {
        ...
        fields.forEach(f => {
            ...
            if (filterValue) {
                filters.push({ type: 'filter', field: f.key, operation: filterOperation ?? (f?.operation ?? Operation.eq), value: filterValue });
            }
                    ...
                    <div className="col-10">
                        {
                            fields.map((f, i) => {
                                return (
                                    <div className="row">
                                        <div className="col-4 h6 mt-3 text-left">
