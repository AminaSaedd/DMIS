...
export const THead: React.FC<Props> = (props) => {
    ...
    const renderCell = (header: ComplexHeader, index: number) => {
        ...
        if (typeof header == 'string') {
            return (<th key={`heading-${index}`}
                className={sortingKey === header ? 'sorting' : ''}
                onClick={() => handleSort(header)}>{header}</th>);
        }

        return (
            <th key={`heading-${header.key}`}
                onClick={() => handleSort(header)}
                className={className + ' ' + (sortingKey === header.key ? 'sorting' : '')}>{header.title ? header.title : header.key}</th>
        );
    };
    ...
    return (
        <thead style={{ position: 'sticky', insetBlockStart: 0, backgroundColor: 'white' }}>
            <tr>
                ...
                {props.hasActions && (<th className="text-center">Actions</th>)}
            </tr>
        </thead>
    );
}